...
sort -k1,1 -uk2,2 ${BASE}sources/ambiguity_data | awk '{ print $1 }' | uniq -c | sort -k1,1n > ${BASE}ambiguity_e

# calculate ambiguity of surface forms by counting the entities for every surface form
# ignoring duplicated entities
sort -k2,2 -uk1,1 ${BASE}sources/ambiguity_data |  awk '{print $2}' | uniq -c | sort -k1,1n > ${BASE}ambiguity_sf
